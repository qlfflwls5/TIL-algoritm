...
for t in range(1, T+1):
    ...
    i = 0
    while i < len(data):
        # 레이저가 아닌 '('를 만나면 막대기가 하나 추가된 것
        if data[i] == '(' and data[i+1] != ')':
            stick += 1
            i += 1
        # 레이저를 만나면 여태까지 있었던 막대기의 개수만큼 조각이 생긴다. 레이저는 두 칸 차지하므로 인덱스를 2 올린다.
        elif data[i] == '(' and data[i+1] == ')':
            cnt_piece += stick
            i += 2
        # 레어지가 아닌 ')'를 만나면 막대기가 끝난 것. 이것은 남은 조각으로 취급해 조각 개수가 1 오른다.
        elif data[i] == ')' and data[i-1] == ')':
            stick -= 1
            cnt_piece += 1
            i += 1

    print('#%d %d' % (t, cnt_piece))
    ...
    #     if data[i] == '(' and data[i+1] != ')':
    #         stack = 1
    #         # 막대기의 여는 괄호('(') 포함 이후의 여는 괄호의 개수와 닫는 괄호의 개수가 같을 때가 막대기의 닫는 괄호이다.
    #         for j in range(i+1, len(data)):
    #             stack += 1 if data[j] == '(' else -1
    #             if stack == 0:
    #                 close_idx = j
    #                 break
    #
    #         # 조각의 개수는 막대기에서 레이저의 개수 +1
    #         cnt_piece += data[i:j+1].count('()') + 1
    #
    # print('#%d %d' %(t, cnt_piece))