...
# 1
# 얘는 왜 어떨 때 에러가 나고 어떨 때는 맞을까?
for t in range(1, int(input())+1):
    N, M = map(int, input().split())
    A = set(input().split())
    B = set(input().split())
    print('#%d %d' % (t, len(A) - len(A-B)))


# 2
# 차이없으나 얘는 무조건 통과다.
# set으로 input을 받아오지 말자. 연산 때만 set으로 바꿔서 사용을 하자.
for t in range(1, int(input())+1):
    N, M = map(int, input().split())
    A = list(input().split())
    B = list(input().split())
    # A의 집합의 길이에서 A 차집합 B의 길이를 빼면 중복의 개수가 나온다.
    print('#%d %d' % (t, len(set(A)) - len(set(A) - set(B))))