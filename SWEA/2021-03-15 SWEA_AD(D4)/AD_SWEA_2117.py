...
for t in range(1, int(input())+1):
    ...
    for i in range(N):
        ...
        # 회사가 손해를 보면 안되므로, 최대 수익(전체 집 수*M) 보다 운영 비용이 크지 않는 범위까지만 확장할 것이다.
        for e in row:
            if e:
                house += 1
        arr[i] = row
        ...
        for j in range(N):
            for k in range(max_K):
                ...
                for r in range(i-k, i+k+1):
                    for c in range(j-abs(abs(r-i)-k), j+abs(abs(r-i)-k)+1):
                        if 0 <= r < N and 0 <= c < N and arr[r][c]:
                            service += 1
                if 0 <= service*M - (k**2 + (k+1)**2) and max_service < service:
                    max_service = service

    print('#%d %d' % (t, max_service))

# 서비스 시작 중심지로부터 마름모를 실제로 그리지말고, 거리만 따져보자
# 서비스 시작 중심지 좌표가 (i, j)이고, 각 집의 좌표를 (home[0], home[1])이라고 하면 거리는 abs(home[0] - i) + abs(home[1] - j)다.
# 그 거리가 k-1 보다 작거나 같으면 서비스 안에 포함되는 것이다.
# 그리고 가지치기를 생각해보자.