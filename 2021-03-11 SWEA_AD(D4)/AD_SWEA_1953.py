...
# 상하좌우를 델타로 표현한 것의 성질을 이용해도 좋다.
# 현재 파이프가 위로 갈 수 있다면 갖고 있는 델타 값은 [-1, 0]이다.
# 그렇다면 위의 파이프가 가져야 할 델타 값은 [1, 0]이다.
# 좌, 우의 쌍도 같은 양상이다.
# 따라서, 내가 갖고 있는 델타 값에 -1을 곱한 것이 그 쪽의 파이프에 존재하면 된다.
# [-1 * -1, 0 * -1] = [1, 0] 위 아래 짝이 맞다.