...
+ 프로그래머스에서 고득점 SQL kit의 STRING, DATE 파트를 풀었다. 
+ 총 5문제의 IN, LIKE, CASE~WHEN~THEN~ELSE~END~AS, DATE_FORMAT, DATEDIFF를 활용한 문제
+ 이번에 새롭게 배울 수 있었던 것은 CASE WHEN THEN ELSE문이다. 이를 통해서 내가 원하는 열을 만들 수 있다.
+ DATE 관련 문제는 함수를 알아야만 풀 수 있다. 많은 함수들을 알아놓자.
+ LIKE에서는 `%`와 `_`를 사용한다. `_`는 한 문자를 의미한다.
+ 느낀점 및 배운점

  + 이제 SQL kit의 모든 문제를 풀었다.
  + 나머지 내용들은 직접 검색해보면서 알아가도록 하자.

<br>

## 2021-11-21

+ 오랜만에 프로젝트를 끝내고 다시 알고리즘으로 돌아와 프로그래머스에서 2021 DEV 채용 상반기 레벨2 문제를 풀었다. 
+ **행렬 테두리 회전하기**
+ 2차원 배열과 좌표를 활용한 문제
  + 좌표를 헷갈리지 않게 잘 표시해야한다.(보통 이런 문제는 행 열이 1부터 시작하므로 0부터 시작하는 프로그래밍에서 신경만 잘써서 맞춰주면 된다.)
  + 특정한 칸이 인접한 다른 칸의 값을 가져오고, 그 특정한 칸에 있던 값은 또 다른 칸으로 이동하는 형태의 알고리즘을 구성해야 하므로 **특정한 칸에 담겨져 있던 값을 계속해서 저장해두고 있어야 할 변수** 하나를 설정하는 것이 중요하다.
  + 시계 방향으로 칸의 값이 이동하는 것이기 때문에 네 귀퉁이(칸의 이동 방향이 바뀌는 지점)를 잘 잘라서 for문으로 처리하면 된다.
+ 느낀점 및 배운점

  + 오랜만에 알고리즘 문제로 다시 돌아오니 정답을 맞추기는 했지만 확실히 약간 감이 떨어진 느낌이 있다.
  + 이제부터 다시 차근차근 시작해본다.
...
+ 프로그래머스에서 2021 DEV 채용 상반기 레벨1 문제를 풀었다. 
+ **로또의 최고 순위와 최저 순위**
+ 집합의 `&`연산을 이용한 문제
  + 내가 찍은 번호의 리스트와 실제 당첨 번호의 리스트가 주어진다.
  + 이 리스트를 set()으로 변환하여 집합 연산을 하면, 몇 개의 숫자가 겹치는지 쉽게 알아낼 수 있다.
+ 느낀점 및 배운점

  + 차근차근 쉬운 문제부터 다시 정복해나간다.
  + 어렵지 않았다.
