...
def solution(s):
    answer = 1000
    # 2분의 1토막까지만 검사하면 된다.
    # 하지만, len(s)//2 + 1까지의 범위로 잡으면 'a'일 때 1이 아니라 1000이 나온다.
    for i in range(1, len(s) // 2 + 2):
        temp_result = ''
        last_word = ''
        cnt = 1
        # 기본적으로 그냥 for문을 돌린다 생각해보면 항상 마지막 시행이 처리가 안되는 문제가 있다.
        # 이에 있어 마지막을 무조건 빈문자열과 비교하는 것으로 처리될 수 있게 리스트 처리를 한다.
        for frag in [s[j:j + i] for j in range(0, len(s), i)] + ['']:
            if last_word != frag:
                temp_result += ('' if cnt == 1 else str(cnt)) + last_word
                last_word = frag
                cnt = 1
            else:
                cnt += 1
