...
# 모든 풍선에는 서로 다른 숫자가 써져 있습니다. 당신은 다음 과정을 반복하면서 풍선들을 단 1개만 남을 때까지 계속 터트리려고 합니다.
...
# 가장 작은 풍선 min 풍선을 찾는다. 이 풍선은 일단 무조건 가능하다.
# 각 풍선에 대해 min 풍선이 오른쪽에 있다면, 왼쪽에 있는 모든 풍선들이 나보다 크면 된다.
# 각 풍선에 대해 min 풍선이 왼쪽에 있다면, 오른쪽에 있는 모든 풍선들이 나보다 크면 된다.
# 가장 빠른 실행속도
def solution(a):
    min_balloon_idx = a.index(min(a))
    min_left, min_right = a[0], a[len(a) - 1]
    answer = 1
    for i in range(min_balloon_idx):
        if a[i] <= min_left:
            answer += 1
            min_left = a[i]

    for i in range(len(a) - 1, min_balloon_idx, -1):
        if a[i] <= min_right:
            answer += 1
            min_right = a[i]
