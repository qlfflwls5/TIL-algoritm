...
def DFS_max(v, cnt):
    ...
    for w in AL[v]:
        if not visited[w]:
            DFS_max(w, cnt + 1)
            # visited[v] = 0    리프 노드에 있는 상태에서 visited 초기화가 아닌 부모로 올라와서 초기화(이렇게 해도 상관없다.
            #                   대신 DFS를 호출하는 for문에서 시작 정점에 대한 visited 처리 필요
    # 리프 노드에 있는 상태에서 바로 visited 초기화
    visited[v] = 0
...
for t in range(1, int(input()) + 1):
    ...
    max_v = [0]
    # 모든 정점에서 다 DFS를 돌려본다.
    # 그리고 visited는 어차피 계속 DFS내에서 해당 정점에 대해 초기화해주므로 for문 안에서 계속 생성할 필요가 없다.
    visited = [0] * (N + 1)
