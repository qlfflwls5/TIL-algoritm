...
# 전체 높이를 카운트 리스트에 넣은 후, 가장 높은 것에서 낮은 것에 하나를 주면
# 카운트 리스트의 인덱스 상 가장 높은 것과 가장 낮은 것은 -1(그 높이를 갖는 상자들이 아니게 되므로) 두 번째로 높은 것과 두 번째로 낮은 것이 +1이 된다.
# 예를 들어, 가장 높은 높이가 5, 낮은 높이가 2라면 5에서 2에 하나를 주면 5와 2의 길이를 갖는 열은 하나가 줄고 4와 3의 높이를 갖는 열이 하나씩 더 생기는 것이다.
# 이를 이용해 가장 낮은 높이나 가장 높은 높이의 카운트가 0이 되면 각각 그 다음으로 이동하면서 작업을 반복한다.
for t in range(1, 11):
    N = int(input())
