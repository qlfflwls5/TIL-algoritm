...
for t in range(1, 11):
    N = int(input())
    # 현재 노드, 문자, 왼쪽 자식 노드, 오른쪽 자식 노드, 부모 노드의 5개 정보를 담는 tree
    tree = [[0]*5 for i in range(N + 1)]
...
# tree에 모든 정보를 담는 것도 좋지만,
# 직관성을 위해 left, right, word, parent 리스트를 따로 만들어서 해당 노드의 인덱스에 정보를 담아도 된다.
